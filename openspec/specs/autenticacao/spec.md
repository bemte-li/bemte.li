...
Como o Bemte.li passa a ter uma área autenticada (`/casa`, `/criar`), precisamos de um fluxo simples de login para os autores. A capacidade `autenticacao` cobre o login de quem já tem uma conta `usuarios` no PocketBase, a persistência de sessão entre reloads, o logout, e o espelhamento da sessão num cookie `pb_auth` para que o middleware do Next.js possa decidir antes do HTML quem entra em rota protegida.

Não há cadastro próprio nem reset de senha aqui — o Bemte.li continua sendo por convite e a criação de conta passa pelo painel do PocketBase.
